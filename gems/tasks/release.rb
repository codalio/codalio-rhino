...
version = File.read("#{root}/CODALIO_VERSION").strip
tag     = "v#{version}"

directory "pkg"

(FRAMEWORKS + ["codalio"]).each do |framework|
  namespace framework do
    ...
    task gem => %w(update_versions pkg) do
      cmd = ""
      cmd += "cd #{framework} && " unless framework == "codalio"
      cmd += "bundle exec rake package && " unless framework == "codalio"
...
namespace :all do
  task build: FRAMEWORKS.map { |f| "#{f}:build"           } + ["codalio:build"]
  task update_versions: FRAMEWORKS.map { |f| "#{f}:update_versions" } + ["codalio:update_versions"]
  task install: FRAMEWORKS.map { |f| "#{f}:install"         } + ["codalio:install"]
  task push: FRAMEWORKS.map { |f| "#{f}:push"            } + ["codalio:push"]

  task :ensure_clean_state do
    files = `git status -s | grep -v 'CODALIO_VERSION\\|CHANGELOG\\|Gemfile.lock\\|package.json\\|version.rb\\|tasks/release.rb'`
